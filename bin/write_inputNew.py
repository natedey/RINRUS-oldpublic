...
#        if lines[1][0] != '#' and 'pm' in lines[1] and lines[2].strip() == 'opt':
    ...
    count = 0
    dblock=['Sc',	'Ti',	'V',	'Cr',	'Mn',	'Fe',	'Co',	'Ni',	'Cu',	'Zn',
    ...
    if lines[10][0] != '#' and 'basis' in lines[10]:
        if args.basis == "get":
            readfrom = newpdb #or 'template.pdb'
            readfrom2 = basisinfo #or 'input_temp'
            basis = []
            with open(f"{readfrom}", "r") as readstart:
                lines = readstart.readlines()
                atomlist = []
                for line in lines:
                    atomname=line.split()
                    if len(line) > 40:
                        atomlist.append(atomname[-2].replace("+","").replace("0","").replace("1","").replace("-",""))
                        used = set()
            ...
            with open(readfrom2, 'r') as fo:
                ...
                for i in range(len(lines)):
                    ...
                    if "****" in line:
                        basis.append(i+1)
                with open ("getbasis","w") as inpwritestart:
                    for i in range(len(lines)):
                        if i in basis:
                            if (lines[i].split(" ")[0].title()) in usednospace:
                                for l in lines[basis[basis.index(i)]:basis[basis.index(i)+1]]:
                                    inpwritestart.writelines(l)
                                    inp.write("%s"%l)
                    inp.write("%s"%"\n")
                for i in dblock:
                    if i in usednospace:
...
#          step2: read outputwrite_modred_inp, input_template, write_new_inp')
    ...
    parser.add_argument('-basis', dest='basis', default=None, help='-basis get (need to have -basisinfo flag')
    parser.add_argument('-basisinfo', dest='basisinfo', default="$HOME/git/RINRUS/template_files/gaussian_input/basisinfo.dat", help='-basisinfo $HOME/git/RINRUS/template_files/gaussian_input/basisinfo.dat\n If -basisinfo is not given then basis set will be taken from intmp')
#    parser.print_help()
